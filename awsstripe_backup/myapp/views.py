    ...
    def get(self,request):
        ...
        if request.user.is_authenticated:
            ...
            try:
                ...
                #self.params['MEDIA_URL'] = settings.IMAGE_ROOT
            except:
            ...
            if request.user.is_authenticated:
                try:
                    weatherplacemodel = WeatherPlaceModel.objects.get(user=request.user)
                    print("weatherplacemodel.weatherplace")
                    print(weatherplacemodel.weatherplace)
                    weather_place= weatherplacemodel.weatherplace
                except:
                    weather_place = "Atami"
            else:
                weather_place = "Atami"
            ...
            self.params['weather_0_icon'] = "http://openweathermap.org/img/wn/" + ret['weather'][0]['icon'] + "@2x.png"
            self.params['weather_name'] = ret['name']
            self.params['weather_main_temp'] = ret['main']['temp']
            ########################################################################################################


            skypegroup = SkypeGroupModel.objects.filter(user=request.user)

            skypeid_join_list = []
            for i in skypegroup:
                #skypegroupmodel = SkypeGroupModel.objects.filter(group_name=skypegroup[0].group_name)
                skypegroupmodel = SkypeGroupModel.objects.filter(group_name=i.group_name)

                skypeid_list = [SkypeMyNameModel.objects.get(user=i.user).skype_id for i in skypegroupmodel]
                skypeid = ';'.join(skypeid_list)
                skypeid_join_list.append([skypeid, i.group_name, GroupNameModel.objects.get(group_name=i.group_name).id])

            print(skypeid_join_list)
            self.params['skypeid'] = skypeid_join_list


            # skypegroupmodel = SkypeGroupModel.objects.filter(group_name="sample")
            # #print("###################skypegroupmodel######################################")
            # #print([SkypeMyNameModel.objects.get(user=i.user).skype_id for i in skypegroupmodel])

            # skypeid_list = [SkypeMyNameModel.objects.get(user=i.user).skype_id for i in skypegroupmodel]
            # skypeid = ';'.join(skypeid_list)
            # self.params['skypeid'] = skypeid
...
class TopGroupView(TemplateView):
    def __init__(self):
        self.params = {}

    #@login_required
    def get(self,request, topgroup_id):
        ...
        if request.user.is_authenticated:
            ...
            for i in skypegroup:
                ...
                skypeid_list = [SkypeMyNameModel.objects.get(user=i.user).skype_id for i in skypegroupmodel]
                skypeid = ';'.join(skypeid_list)
                skypeid_join_list.append([skypeid, i.group_name, i.id])

            print(skypeid_join_list)
            self.params['skypeid'] = skypeid_join_list
            ########################################################################################################

            skypegroup_name = GroupNameModel.objects.get(id=topgroup_id)

            skypegroup = SkypeGroupModel.objects.filter(group_name=skypegroup_name)
            ...
            for i in skypegroup:
                if not i.user.id == request.user.id:
                    print("##############i##############")
                    print(i.id)
                    user_arr = SkypeMyNameModel.objects.get(user=i.user)
                    skypeid_list.append(user_arr)

            print(skypeid_list)
            self.params['sample'] = skypeid_list

            ########################################################################################################

            return render(request,'myapp/homegroup.html',self.params)

        #return render(request,'myapp/home.html',self.params)
        return render(request,'myapp/home_logout.html',self.params)

    def post(self, request, topgroup_id):
        return render(request,'myapp/home.html',self.params)
...
class AddUserInfoView(TemplateView):
    def __init__(self):
        self.params = {}

    #@login_required
    def get(self, request):
        self.params["form"] = UploadForm()
        self.params["userform"] = UserForm()

        #self.params["weatherform"] = WeatherForm()

        return render(request,'myapp/add_user_info.html',self.params)

    def post(self, request):
        # print("###########request.POST.get('userimages')###########")
        # print(request.POST.get('userimages'))
        # #20230123
        # user_info = SkypeMyNameModel.objects.create(
        ...
        # if form.is_valid():
        #     form.user = request.user
        #     form.skype_id = request.POST.get('skypeid'),
...
############AddUserWeatherInfoView##########################################################################################
class AddUserWeatherInfoView(TemplateView):
    def __init__(self):
        self.params = {}
    ...
    def post(self, request):
        ...
        print(request.POST.get('weatherplace'))
        #user_obj=User.objects.get(id=request.user.id)
        weatherplacemodel = WeatherPlaceModel.objects.create(
            user=User.objects.get(id=request.user.id),
            weatherplace=request.POST.get('weatherplace')
        )

        return render(request,'myapp/add_user_info.html',self.params)

class AddGroupNameInfoView(TemplateView):
    def __init__(self):
        self.params = {}
    ...
    def post(self, request):
        ...
        #user_obj=User.objects.get(id=request.user.id)
        groupnamemodel = GroupNameModel.objects.create(
            group_name=request.POST.get('groupname')
        )

        return render(request,'myapp/add_user_info.html',self.params)

class AddGroupUserInfoView(TemplateView):
    def __init__(self):
        self.params = {}
    ...
    def post(self, request):
        ...
        group_name_request=request.POST.get('groupname')
        # print(group_name_request)
        # print(GroupNameModel.objects.get(group_name=group_name_request))

        groupnamemodel = SkypeGroupModel.objects.create(
            group_name=GroupNameModel.objects.get(group_name=group_name_request),
            user=User.objects.get(id=request.user.id),
        )
